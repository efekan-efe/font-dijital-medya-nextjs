...
import FontLogo from "../components/icons/FontLogo";
...
export const StaggeredMenu = ({
  ...
  const [textLines, setTextLines] = useState(["Menü", "Kapat"]);
    ...
    (callback) => {
      ...
      closeTweenRef.current = gsap.to(all, {
        ...
        onComplete: () => {
          ...
          busyRef.current = false;
          callback?.(); // <-- 2. Geri aramayı burada (en sonda) çağırın
        },
      });
    },
    [position]
  ...
  const toggleMenu = useCallback(() => {
    ...
    if (target) {
      ...
      playClose(onMenuClose); // <-- ÇÖZÜM: 'onMenuClose' prop'unu callback olarak 'playClose'a geçirin
    }
  ...
  return (
    <div className={`sm-scope font-inter z-40 border-b border-gray-200 ${isFixed ? "fixed top-0 left-0 w-screen h-screen overflow-hidden" : "w-full h-full"}`}>
      <div className={(className ? className + " " : "") + "staggered-menu-wrapper relative w-full h-full"} style={accentColor ? { ["--sm-accent"]: accentColor } : undefined} data-position={position} data-open={open || undefined}>
        ...
        <header className="staggered-menu-header absolute top-0 left-0 w-full flex items-center justify-between p-[2em] bg-transparent pointer-events-none z-20" aria-label="Main navigation header">
          <Link className="w-full h-full max-w-[120px]" href="/">
            <FontLogo />
          </Link>
        ...
        >
          <div className="sm-panel-inner flex-1 flex flex-col gap-5">
              ...
              {items && items.length ? (
                items.map((it, idx) => (
                  <li className="sm-panel-itemWrap relative overflow-hidden leading-none" key={it.label + idx}>
                    <Link
                      className="sm-panel-item relative text-black font-semibold text-[3.25rem] cursor-pointer leading-none tracking-[-2px] transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]"
                      href={it.link}
                      aria-label={it.label} // <-- Düzeltme 1: MobileHeader'daki prop ile eşleşti
                      data-index={idx + 1}
                      onClick={toggleMenu} // <-- DÜZELTME 2: (ANA ÇÖZÜM) Menü kapatma fonksiyonu eklendi
                    >
                ...
                <li className="sm-panel-itemWrap relative overflow-hidden leading-none" aria-hidden="true">
                  <span className="sm-panel-item relative text-black font-semibold text-[3.25rem] cursor-pointer leading-none tracking-[-2px] transition-[background,color] duration-150 ease-linear inline-block no-underline pr-[1.4em]">
            ...
            {displaySocials && socialItems && socialItems.length > 0 && (
              <div className="sm-socials pt-8 flex flex-col gap-3 !mt-8" aria-label="Social links">
...
position: absolute; top: 0; right: 0; width: clamp(260px, 38vw, 420px); height: 100%; background: white; backdrop-filter: blur(12px); -webkit-backdrop-filter: blur(12px); display: flex; flex-direction: column; padding: 6em 2em 2em 2em; overflow-y: auto; z-index: 10; }
...
.sm-scope .sm-panel-title { margin: 0; font-size: 1rem; font-weight: 600; color: #fff;  }
.sm-scope .sm-panel-list { list-style: none; margin: 0; padding: 0; display: flex; flex-direction: column; gap: 0.5rem; }
.sm-scope .sm-panel-item { position: relative; color: #000; font-weight: 600; font-size: 3.25rem; cursor: pointer; line-height: 1; letter-spacing: -2px;  transition: background 0.25s, color 0.25s; display: inline-block; text-decoration: none; padding-right: 1.4em; }
...
@media (max-width: 1024px) { 
  ...
  .sm-scope .sm-prelayers { width: 100%; left: 0; right: 0; } /* <-- BU SATIRI EKLEYİN */
