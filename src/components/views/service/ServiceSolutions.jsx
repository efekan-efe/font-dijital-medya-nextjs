...
const ServiceSolutions = ({ solutionData }) => {
  return (
    <div className="max-w-7xl mx-auto flex flex-col items-center justify-center gap-12 px-2">
      ...
      <div className="w-full flex justify-start items-start flex-wrap gap-3 gap-y-6">
        {solutionData.map((solution, index) => (
          <RuleRow className="min-w-[400px] w-full max-[450px]:min-w-0" key={index} number={index + 1} title={solution.title} description={solution.description} />
        ))}
      </div>
    </div>
  );
